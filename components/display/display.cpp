    ...
    for (int x = 0; x != w; x++, source_idx++) {
      switch (bitness) {
        ...
        case COLOR_BITNESS_888:
          ...
          break;
        // LVGL XRGB8888 at LV_COLOR_DEPTH=32: memory [B, G, R, X] on little-endian.
        // Pack into a uint32_t that ColorUtil::to_color(COLOR_BITNESS_8888) can decode.
        case COLOR_BITNESS_8888:
          source_idx_mod = source_idx * 4;
          color_value = ptr[source_idx_mod + 0] |                     // B
                        (ptr[source_idx_mod + 1] << 8) |              // G
                        (ptr[source_idx_mod + 2] << 16);              // R  (X byte ignored)
          break;
      }
