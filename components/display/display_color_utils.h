...
enum ColorBitness : uint8_t {
  ...
  COLOR_BITNESS_565 = 1,
  COLOR_BITNESS_332 = 2,
  // 4 bytes/pixel ARGB8888/XRGB8888 as produced by LVGL at LV_COLOR_DEPTH=32.
  // Memory layout on little-endian: [B, G, R, A]. The 4th byte (alpha or X)
  // is ignored by display drivers that only handle opaque RGB output.
  COLOR_BITNESS_8888 = 3,
};
...
class ColorUtil {
 public:
  static Color to_color(uint32_t colorcode, ColorOrder color_order,
    ...
    // XRGB8888: memory layout [B, G, R, X] on little-endian → colorcode is 0x00RRGGBB after masking.
    // Handle before the generic first/second/third_bits path so the X byte is never confused
    // for a color channel.
    if (color_bitness == COLOR_BITNESS_8888) {
      Color c;
      c.b = colorcode & 0xFF;
      c.g = (colorcode >> 8) & 0xFF;
      c.r = (colorcode >> 16) & 0xFF;
      c.w = 0;
      return c;
    }
