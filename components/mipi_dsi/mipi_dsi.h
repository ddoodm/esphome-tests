...
class MIPI_DSI : public display::Display {
 public:
  ...
  ~MIPI_DSI() {
    if (this->xrgb_scratch_ != nullptr) {
      free(this->xrgb_scratch_);  // NOLINT
      this->xrgb_scratch_ = nullptr;
    }
  }
 ...
 protected:
  ...
  // Reusable RGB888 staging buffer for ARGB8888→RGB888 conversion in
  // draw_pixels_at. Allocated on first use, grown on demand. Holds the
  // converted output of one full draw_pixels_at call (w * h * 3 bytes) so
  // we can submit a single esp_lcd_panel_draw_bitmap and a single
  // xSemaphoreTake instead of one per row.
  uint8_t *xrgb_scratch_{nullptr};
  size_t xrgb_scratch_size_{0};
  bool logged_strip_path_{false};
  uint16_t x_low_{1};
  uint16_t y_low_{1};
  uint16_t x_high_{0};
  uint16_t y_high_{0};
