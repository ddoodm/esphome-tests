...
void MIPI_DSI::draw_pixels_at(int x_start, int y_start, int w, int h, const uint8_t *ptr, display::ColorOrder order,
  ...
  // Fast path: source bitness matches the DPI framebuffer depth — blit directly.
  ...
  // ARGB8888 / XRGB8888 → RGB888: LVGL with LV_COLOR_DEPTH=32 produces
  // 4-byte-per-pixel buffers (memory order [B, G, R, A/X] on little-endian P4).
  // The DPI framebuffer is RGB888 (3 bytes/pixel). Strip the alpha byte into a
  // reusable staging buffer sized to the full area, then submit ONE
  // esp_lcd_panel_draw_bitmap and wait on ONE semaphore — mirroring the
  // fast-path behaviour in write_to_display_.
  //
  // Per-row submission used to live here but caused two problems under burst
  // load (rapid touch drags):
  //   1. Each row was an async DMA submission + portMAX_DELAY semaphore wait.
  //      If a submission ever errored, the callback never fired and the loop
  //      task deadlocked, tripping the task WDT after 30s.
  //   2. Even when successful, the per-row overhead (window setup, DMA queue,
  //      wait) dominated and made the strip ~hundreds-of-times slower than a
  //      single bulk DMA.
  if (bitness == display::COLOR_BITNESS_8888 && this->color_depth_ == display::COLOR_BITNESS_888) {
    // Marker so we can verify the loaded firmware has the bounded-timeout strip path.
    // Fires exactly once per boot, regardless of how many strips we end up doing.
    if (!this->logged_strip_path_) {
      ESP_LOGI(TAG, "ARGB8888 -> RGB888 strip path engaged (bounded-timeout)");
      this->logged_strip_path_ = true;
    }
    const size_t needed = (size_t) w * (size_t) h * 3;
    if (this->xrgb_scratch_ == nullptr || this->xrgb_scratch_size_ < needed) {
      free(this->xrgb_scratch_);  // NOLINT — free(nullptr) is safe
      // Use heap_caps_malloc to prefer PSRAM; the scratch can be hundreds of
      // KB on full-screen redraws and we don't want to eat DRAM.
      this->xrgb_scratch_ = static_cast<uint8_t *>(heap_caps_malloc(needed, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT));
      if (this->xrgb_scratch_ == nullptr) {
        // Fall back to default heap if SPIRAM allocation failed.
        this->xrgb_scratch_ = static_cast<uint8_t *>(malloc(needed));  // NOLINT
      }
      if (this->xrgb_scratch_ == nullptr) {
        ESP_LOGE(TAG, "draw_pixels_at: failed to allocate XRGB scratch (%u bytes)", (unsigned) needed);
        return;
      }
      this->xrgb_scratch_size_ = needed;
    }

    // Source stride in pixels (includes any left/right offsets).
    const int src_stride_px = x_offset + w + x_pad;
    const bool bgr = (this->color_mode_ == display::COLOR_ORDER_BGR);

    // Strip the entire w×h area into the scratch buffer.
    uint8_t *dst = this->xrgb_scratch_;
    for (int y = 0; y != h; y++) {
      const uint8_t *src = ptr + ((y_offset + y) * src_stride_px + x_offset) * 4;
      for (int x = 0; x != w; x++, src += 4, dst += 3) {
        // LVGL ARGB8888 little-endian: src[0]=B, src[1]=G, src[2]=R, src[3]=A (ignored).
        if (bgr) {
          dst[0] = src[0];  // B
          dst[1] = src[1];  // G
          dst[2] = src[2];  // R
    ...
    // Robustly synchronise with the async DMA copy from xrgb_scratch_ →
    // DPI framebuffer.
    //
    // The on_color_trans_done callback fires from an ISR and gives a binary
    // semaphore (io_lock_). Two failure modes in the prior approach (wait
    // unbounded after submit) caused the 30s task WDT to trip during rapid
    // LVGL invalidations:
    //
    //   * If esp_lcd_panel_draw_bitmap() returned ESP_ERR_INVALID_STATE
    //     because the previous DMA was still in flight, no callback fired
    //     for THIS call but we still waited on the semaphore → deadlock.
    //   * If the callback fired between two of our calls (i.e. while we
    //     weren't waiting), the binary semaphore would clamp at 1.  A later
    //     wait would then take that stale give immediately, even though the
    //     current DMA hadn't completed — buffer-corruption + missed-wake.
    //
    // Fix: drain any stale give from the semaphore BEFORE submit, then submit,
    // then wait with a bounded timeout. The drain guarantees that after
    // submit, the only way the semaphore is ever given again is by THIS
    // DMA's completion. The bounded wait keeps the loop task alive even if
    // the callback somehow gets dropped.
    while (xSemaphoreTake(this->io_lock_, 0) == pdTRUE) {
      // discard stale signals — there's at most one queued (binary semaphore)
    }

    esp_err_t err =
        esp_lcd_panel_draw_bitmap(this->handle_, x_start, y_start, x_start + w, y_start + h, this->xrgb_scratch_);
    if (err != ESP_OK) {
      ESP_LOGW(TAG, "draw_pixels_at (argb strip): esp_lcd_panel_draw_bitmap failed: %s", esp_err_to_name(err));
      return;
    }
    if (xSemaphoreTake(this->io_lock_, pdMS_TO_TICKS(100)) != pdTRUE) {
      // DMA completion didn't fire within the budget. Subsequent calls will
      // return ESP_ERR_INVALID_STATE until the DMA actually finishes (handled
      // above); the loop task survives to render the next frame.
      ESP_LOGW(TAG, "draw_pixels_at (argb strip): on_color_trans_done timeout (100ms)");
    }
    return;
  }

  // Fallback: let the base class convert pixel-by-pixel via draw_pixel_at.
