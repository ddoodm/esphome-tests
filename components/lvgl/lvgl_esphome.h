...
// LVGL 9 at depth 32 produces ARGB8888: 4 bytes/pixel, layout [B, G, R, A] on little-endian.
// (We use ARGB8888 rather than XRGB8888 because ESPHome's lv_conf.h only enables the ARGB path
// in the SW renderer. PPA's ARGB8888 SRM mode treats the two formats identically.)
// COLOR_BITNESS_8888 signals the display driver to read 4 bytes and strip the alpha channel.
static const display::ColorBitness LV_BITNESS = display::ColorBitness::COLOR_BITNESS_8888;
#else   // LV_COLOR_DEPTH
...
class LvglComponent : public PollingComponent {
 ...
 protected:
  ...
  // prevents repeatedly waiting full 100ms timeouts per frame and lets the
  // loop task make progress until PPA recovers.
  uint32_t ppa_skip_until_ms_{0};
  uint32_t ppa_last_warn_ms_{0};
#endif
};
