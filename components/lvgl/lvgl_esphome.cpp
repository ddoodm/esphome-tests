...
bool LvglComponent::ppa_rotate_(const lv_color_data *src, lv_color_data *dst, uint16_t width, uint16_t height,
  ...
  // Saturation backoff. Under heavy LVGL load (rapid touch invalidations at
  // LV_COLOR_DEPTH=32), PPA contends with the DPI panel's async DMA2D copy
  // and its completion ISRs back up. Rather than wait the full per-call
  // timeout (and stack up timeouts across many calls), short-circuit to the
  // CPU rotation path for a brief cooldown after a PPA stall. PPA re-engages
  // automatically when the cooldown expires.
  const uint32_t now = millis();
  if (this->ppa_skip_until_ms_ != 0 && (int32_t)(now - this->ppa_skip_until_ms_) < 0) {
    return false;
  }
  ...
  // BLOCKING mode internally waits on a semaphore with portMAX_DELAY.  Under
  // heavy LVGL load (rapid touch invalidations at LV_COLOR_DEPTH=32) we have
  // observed the loop task locked here long enough to trip the 30s task WDT,
  ...
  while (xSemaphoreTake(this->ppa_done_sem_, 0) == pdTRUE) {
  }

  // Engage a longer cooldown when PPA is misbehaving. 500ms is enough for the
  // DPI panel's pending DMA copies to drain so PPA can re-acquire DMA2D.
  constexpr uint32_t SATURATION_COOLDOWN_MS = 500;
  // Rate-limit the log messages — under stall, the same warning would otherwise
  // fire 20+ times per second.
  constexpr uint32_t WARN_THROTTLE_MS = 1000;

  esp_err_t ret = ppa_do_scale_rotate_mirror(this->ppa_client_, &srm_config);
  if (ret != ESP_OK) {
    this->ppa_skip_until_ms_ = millis() + SATURATION_COOLDOWN_MS;
    if ((int32_t)(millis() - this->ppa_last_warn_ms_) >= (int32_t) WARN_THROTTLE_MS) {
      ESP_LOGW(TAG, "PPA rotation saturated (submit %s) — using CPU rotation for %ums",
               esp_err_to_name(ret), (unsigned) SATURATION_COOLDOWN_MS);
      this->ppa_last_warn_ms_ = millis();
    }
    return false;
  }
  ...
  if (xSemaphoreTake(this->ppa_done_sem_, pdMS_TO_TICKS(50)) != pdTRUE) {
    ...
    // transaction stays queued in IDF and will recycle its slot when it
    // eventually finishes (our drain on the next call sweeps the stale give).
    this->ppa_skip_until_ms_ = millis() + SATURATION_COOLDOWN_MS;
    if ((int32_t)(millis() - this->ppa_last_warn_ms_) >= (int32_t) WARN_THROTTLE_MS) {
      ESP_LOGW(TAG, "PPA rotation saturated (50ms timeout) — using CPU rotation for %ums",
               (unsigned) SATURATION_COOLDOWN_MS);
      this->ppa_last_warn_ms_ = millis();
    }
    return false;
  }
...
void LvglComponent::setup() {
  ...
  // Use ARGB8888 rather than the depth-32 "native" XRGB8888, because ESPHome's
  // lv_conf.h emits LV_DRAW_SW_SUPPORT_XRGB8888=0 but LV_DRAW_SW_SUPPORT_ARGB8888=1.
  // Memory layout is identical ([B, G, R, A]); the alpha byte is preserved by the
  // PPA SRM ARGB8888 path and ignored by the mipi_dsi driver's strip-on-blit.
  ...
  if (this->rotation_type_ == RotationType::ROTATION_SOFTWARE) {
    ...
    if (ppa_register_client(&ppa_config, &this->ppa_client_) != ESP_OK) {
      ...
      // Create the per-rotation completion semaphore + register the SRM done
      // callback. We run PPA in NON_BLOCKING mode and wait on this semaphore
      // ourselves so we can apply a timeout (see ppa_rotate_).
      this->ppa_done_sem_ = xSemaphoreCreateBinary();
      if (this->ppa_done_sem_ == nullptr) {
        ESP_LOGW(TAG, "PPA semaphore allocation failed, using software rotation");
        ppa_unregister_client(this->ppa_client_);
        this->ppa_client_ = nullptr;
      } else {
        ppa_event_callbacks_t cbs{};
        cbs.on_trans_done = LvglComponent::ppa_trans_done_cb;
        if (ppa_client_register_event_callbacks(this->ppa_client_, &cbs) != ESP_OK) {
          ESP_LOGW(TAG, "PPA callback registration failed, using software rotation");
