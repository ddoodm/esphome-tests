...
lv_image_dsc_t *Image::get_lv_image_dsc() {
  ...
  if (this->dsc_.data != this->data_start_) {
    ...
    switch (this->get_type()) {
      ...
      case IMAGE_TYPE_RGB:
        // IMAGE_TYPE_RGB images are stored as packed RGB888 (3 bytes/pixel) on disk,
        // or ARGB8888 (4 bytes/pixel) when an alpha channel is present. The on-disk
        // layout is independent of the LVGL framebuffer depth, so use the same
        // mapping for both LV_COLOR_DEPTH=16 and =32. (LVGL's SW blender supports
        // RGB888 and ARGB8888 sources but NOT XRGB8888 unless
        // LV_DRAW_SW_SUPPORT_XRGB8888 is enabled, which ESPHome's lv_conf.h leaves off.)
        this->dsc_.header.cf =
            this->transparency_ == TRANSPARENCY_ALPHA_CHANNEL ? LV_COLOR_FORMAT_ARGB8888 : LV_COLOR_FORMAT_RGB888;
        break;

      case IMAGE_TYPE_RGB565:
        // RGB565 images always carry 565 data regardless of the LVGL color depth.
        // Use v9 canonical names; avoid LV_IMG_CF_RGB565 which is a depth=16-only compat alias.
        this->dsc_.header.cf =
